...
@school_group_router.post("/create_group", summary="Create a new school group")
async def create_school_group(payload: SchoolGroupCreateRequest, db: AsyncSession = Depends(get_db)):
    # duplicate check: same school_id + group_name
...
@school_group_router.get("/grouplist", summary="List all school groups")
async def list_school_groups(
    school_id: int | None = Query(None),
    page:      int        = Query(1,    ge=1),
    limit:     int        = Query(10,   ge=1, le=100),
    search:    str | None = Query(None, description="Search by group_name, or type 'active'/'inactive' to filter by status"),
    db: AsyncSession = Depends(get_db),
):
    key = _list_key(page, limit, search)
    cached = await cache.get(key)
    if cached is not None:
        return Result(code=200, message="School groups fetched successfully.", extra=cached).http_response()
...
@school_group_router.get("/get_id/{group_id}", summary="Get a school group by ID")
async def get_school_group(group_id: int, db: AsyncSession = Depends(get_db)):
    key = _group_key(group_id)
    cached = await cache.get(key)
    if cached is not None:
        return Result(code=200, message="School group fetched successfully.", extra=cached).http_response()
...
@school_group_router.put("/update_group/{group_id}", summary="Update a school group")
...
@school_group_router.delete("/delete_group/{group_id}", summary="Soft delete a school group and all its children")
async def delete_school_group(group_id: int, db: AsyncSession = Depends(get_db)):
    ...
    class_ids = (await db.execute(
        select(SchoolStreamClass.class_id).where(SchoolStreamClass.school_group_id == group_id)
    )).scalars().all()
...
@school_group_router.get("/school-groups/all", summary="Dropdown: School Groups")

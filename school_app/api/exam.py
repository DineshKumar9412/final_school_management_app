...
@exam_router.post("/grade/create", summary="Create a grade",
    responses={
...
async def create_grade(payload: GradeCreate, db: AsyncSession = Depends(get_db)):
    exists = await db.execute(
        select(Grade.grade_id).where(Grade.start_range == payload.start_range, Grade.end_range == payload.end_range)
    )
    ...
    data = {"grade_id": grade.grade_id, "start_range": float(grade.start_range), "end_range": float(grade.end_range), "grade": grade.grade, "is_active": grade.is_active}
    await cache.set(f"grade:{grade.grade_id}", data, expire=CACHE_TTL)
    await cache.delete_pattern("grade:list:*")
    return Result(code=201, message="Grade created successfully.", extra=data).http_response()


@exam_router.post("/grade/bulk_create", summary="Bulk create grades",
    responses={
...
async def create_grades_bulk(payload: List[GradeCreate], db: AsyncSession = Depends(get_db)):
    ...
    existing = (await db.execute(select(Grade.start_range, Grade.end_range).where(tuple_(Grade.start_range, Grade.end_range).in_(ranges)))).all()
    if existing:
...
@exam_router.get("/grade/list", summary="List all grades (paginated)")
async def list_grades(
    ...
    data = {
        ...
        "data": [{"grade_id": g.grade_id, "start_range": float(g.start_range), "end_range": float(g.end_range), "grade": g.grade, "is_active": g.is_active} for g in rows],
    }
    if total > 0:
...
@exam_router.get("/grade/get_id/{grade_id}", summary="Get grade by ID")
async def get_grade(grade_id: int, db: AsyncSession = Depends(get_db)):
    ...
    grade = (await db.execute(select(Grade).where(Grade.grade_id == grade_id))).scalar_one_or_none()
    if not grade:
...
@exam_router.delete("/grade/delete/{grade_id}", summary="Delete a grade")
async def delete_grade(grade_id: int, db: AsyncSession = Depends(get_db)):
    ...
    await db.commit()
    await cache.delete(f"grade:{grade_id}")
    await cache.delete_pattern("grade:list:*")
...
@exam_router.post("/exam/create", summary="Create an exam")
async def create_exam(payload: ExamCreate, db: AsyncSession = Depends(get_db)):
    if (await db.execute(select(Exam.exam_id).where(Exam.exam_name == payload.exam_name))).scalar_one_or_none():
    ...
    data = {"exam_id": exam.exam_id, "exam_name": exam.exam_name, "class_id": exam.class_id, "session_yr": exam.session_yr, "exam_description": exam.exam_description, "is_active": exam.is_active}
    await cache.set(f"exam:{exam.exam_id}", data, expire=CACHE_TTL)
    await cache.delete_pattern("exam:list:*")
    return Result(code=201, message="Exam created successfully.", extra=data).http_response()


@exam_router.get("/exam/list", summary="List exams")
async def list_exams(
    ...
    if search is not None and search.lower() in {"active", "inactive"}:
        ...
        stmt = stmt.where(or_(Exam.exam_name.like(f"%{search}%"), Exam.session_yr.like(f"%{search}%")))
    else:
    ...
    data = {
        "total": total, "page": page, "limit": limit,
        "data": [{"exam_id": e.exam_id, "exam_name": e.exam_name, "class_id": e.class_id, "session_yr": e.session_yr, "exam_description": e.exam_description, "is_active": e.is_active} for e in rows],
    }
    if total > 0:
        await cache.set(key, data, expire=CACHE_TTL)
...
@exam_router.get("/exam/get_id/{exam_id}", summary="Get exam by ID")
async def get_exam(exam_id: int, db: AsyncSession = Depends(get_db)):
    ...
    exam = (await db.execute(select(Exam).where(Exam.exam_id == exam_id))).scalar_one_or_none()
    if not exam:
...
@exam_router.put("/exam/update/{exam_id}", summary="Update an exam")
...
@exam_router.delete("/exam/delete/{exam_id}", summary="Delete an exam")
...
@exam_router.post("/exam/timetable/create", summary="Create exam timetable")
async def create_exam_timetable(payload: ExamTimetableCreate, db: AsyncSession = Depends(get_db)):
    if (await db.execute(select(ExamTimetable.timetable_id).where(ExamTimetable.exam_id == payload.exam_id))).scalar_one_or_none():
...
@exam_router.get("/exam/timetable/list", summary="List exam timetables")
async def list_exam_timetables(
    ...
    if class_id: stmt = stmt.where(ExamTimetable.class_id == class_id)
    if exam_id:  stmt = stmt.where(ExamTimetable.exam_id  == exam_id)
    if search is not None and search.lower() in {"active", "inactive"}:
    ...
    data = {
        "total": total, "page": page, "limit": limit,
        "data": [{"timetable_id": t.timetable_id, "exam_id": t.exam_id, "class_id": t.class_id, "school_group_id": t.school_group_id, "subject_id": t.subject_id, "total_marks": float(t.total_marks) if t.total_marks else None, "pass_mark": float(t.pass_mark) if t.pass_mark else None, "exam_start_date": str(t.exam_start_date) if t.exam_start_date else None, "exam_end_date": str(t.exam_end_date) if t.exam_end_date else None, "is_active": t.is_active} for t in rows],
    }
    if total > 0:
        await cache.set(key, data, expire=CACHE_TTL)
...
@exam_router.put("/exam/timetable/update/{timetable_id}", summary="Update exam timetable")
async def update_exam_timetable(timetable_id: int, payload: ExamTimetableUpdate, db: AsyncSession = Depends(get_db)):
    timetable = (await db.execute(select(ExamTimetable).where(ExamTimetable.timetable_id == timetable_id))).scalar_one_or_none()
    if not timetable:
        return Result(code=404, message="Timetable not found.", extra={}).http_response()

    if payload.exam_id:
        dup = (await db.execute(select(ExamTimetable.timetable_id).where(ExamTimetable.exam_id == payload.exam_id, ExamTimetable.timetable_id != timetable_id))).scalar_one_or_none()
        if dup:
...
@exam_router.delete("/exam/timetable/delete/{timetable_id}", summary="Delete exam timetable")
...
@exam_router.post("/marks/create", summary="Create student marks (bulk per student)")
async def create_marks(payload: StudentMarksCreate, db: AsyncSession = Depends(get_db)):
    ...
    existing = set((await db.execute(select(StudentMarks.subject_id).where(StudentMarks.student_id == payload.student_id, StudentMarks.subject_id.in_(subject_ids)))).scalars().all())
    if existing:
...
@exam_router.post("/online_exam/create", summary="Create online exam")
async def create_online_exam(payload: OnlineExamCreate, db: AsyncSession = Depends(get_db)):
    ...
    if payload.exam_code:
        if (await db.execute(select(OnlineExam.id).where(OnlineExam.exam_code == payload.exam_code))).scalar_one_or_none():
    ...
    data = {"id": exam.id, "title": exam.title, "class_id": exam.class_id, "subject_id": exam.subject_id, "exam_code": exam.exam_code, "url": exam.url, "duration": exam.duration, "start_date": str(exam.start_date), "end_date": str(exam.end_date)}
    await cache.set(f"online_exam:{exam.id}", data, expire=CACHE_TTL)
    await cache.delete_pattern("online_exam:list:*")
    return Result(code=201, message="Online exam created successfully.", extra=data).http_response()


@exam_router.get("/online_exam/list", summary="List online exams")
async def list_online_exams(
    ...
    if search:
        stmt = stmt.where(or_(OnlineExam.title.like(f"%{search}%"), OnlineExam.exam_code.like(f"%{search}%")))
    ...
    data = {
        "total": total, "page": page, "limit": limit,
        "data": [{"id": e.id, "title": e.title, "class_id": e.class_id, "subject_id": e.subject_id, "exam_code": e.exam_code, "url": e.url, "duration": e.duration, "start_date": str(e.start_date), "end_date": str(e.end_date)} for e in rows],
    }
    if total > 0:
        await cache.set(key, data, expire=CACHE_TTL)
...
@exam_router.put("/online_exam/update/{exam_id}", summary="Update online exam")
async def update_online_exam(exam_id: int, payload: OnlineExamUpdate, db: AsyncSession = Depends(get_db)):
    exam = (await db.execute(select(OnlineExam).where(OnlineExam.id == exam_id))).scalar_one_or_none()
    if not exam:
        return Result(code=404, message="Online exam not found.", extra={}).http_response()

    update_data = payload.model_dump(exclude_unset=True)
    if update_data.get("end_date", exam.end_date) < update_data.get("start_date", exam.start_date):
        return Result(code=400, message="End date must be >= start date.", extra={}).http_response()
    ...
    data = {"id": exam.id, "title": exam.title, "class_id": exam.class_id, "subject_id": exam.subject_id, "exam_code": exam.exam_code, "url": exam.url, "duration": exam.duration, "start_date": str(exam.start_date), "end_date": str(exam.end_date)}
    await cache.set(f"online_exam:{exam_id}", data, expire=CACHE_TTL)
    await cache.delete_pattern("online_exam:list:*")
    return Result(code=200, message="Online exam updated successfully.", extra=data).http_response()


@exam_router.delete("/online_exam/delete/{exam_id}", summary="Delete online exam")
async def delete_online_exam(exam_id: int, db: AsyncSession = Depends(get_db)):
    ...
    await db.delete(exam)
    await db.commit()
    await cache.delete(f"online_exam:{exam_id}")
    await cache.delete_pattern("online_exam:list:*")
...
@exam_router.post("/online_class/create", summary="Create online class")
async def create_online_class(payload: OnlineClassCreate, db: AsyncSession = Depends(get_db)):
    ...
    data = {"id": obj.id, "title": obj.title, "class_id": obj.class_id, "subject_id": obj.subject_id, "url": obj.url, "duration": obj.duration, "start_date": str(obj.start_date), "end_date": str(obj.end_date)}
    await cache.set(f"online_class:{obj.id}", data, expire=CACHE_TTL)
    await cache.delete_pattern("online_class:list:*")
    return Result(code=201, message="Online class created successfully.", extra=data).http_response()


@exam_router.get("/online_class/list", summary="List online classes")
async def list_online_classes(
    ...
    if search:
        stmt = stmt.where(or_(OnlineClass.title.like(f"%{search}%"), OnlineClass.url.like(f"%{search}%")))
    ...
    data = {
        "total": total, "page": page, "limit": limit,
        "data": [{"id": c.id, "title": c.title, "class_id": c.class_id, "subject_id": c.subject_id, "url": c.url, "duration": c.duration, "start_date": str(c.start_date), "end_date": str(c.end_date)} for c in rows],
    }
    if total > 0:
        await cache.set(key, data, expire=CACHE_TTL)
...
@exam_router.put("/online_class/update/{online_class_id}", summary="Update online class")
async def update_online_class(online_class_id: int, payload: OnlineClassUpdate, db: AsyncSession = Depends(get_db)):
    obj = (await db.execute(select(OnlineClass).where(OnlineClass.id == online_class_id))).scalar_one_or_none()
    if not obj:
        return Result(code=404, message="Online class not found.", extra={}).http_response()

    update_data = payload.model_dump(exclude_unset=True)
    if update_data.get("end_date", obj.end_date) < update_data.get("start_date", obj.start_date):
        return Result(code=400, message="End date must be >= start date.", extra={}).http_response()
    ...
    data = {"id": obj.id, "title": obj.title, "class_id": obj.class_id, "subject_id": obj.subject_id, "url": obj.url, "duration": obj.duration, "start_date": str(obj.start_date), "end_date": str(obj.end_date)}
    await cache.set(f"online_class:{online_class_id}", data, expire=CACHE_TTL)
    await cache.delete_pattern("online_class:list:*")
    return Result(code=200, message="Online class updated successfully.", extra=data).http_response()


@exam_router.delete("/online_class/delete/{online_class_id}", summary="Delete online class")
async def delete_online_class(online_class_id: int, db: AsyncSession = Depends(get_db)):
    ...
    await db.delete(obj)
    await db.commit()
    await cache.delete(f"online_class:{online_class_id}")
    await cache.delete_pattern("online_class:list:*")

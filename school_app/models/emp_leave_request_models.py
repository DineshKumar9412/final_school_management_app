...
from sqlalchemy import String, BigInteger, DateTime, Enum, ForeignKey, func, LargeBinary
...
class EmpLeaveRequest(Base):
    ...
    type:        Mapped[Optional[LeaveTypeEnum]]  = mapped_column(Enum(LeaveTypeEnum, values_callable=lambda x: [e.value for e in x], native_enum=False), nullable=True)
    status:      Mapped[Optional[LeaveStatusEnum]]= mapped_column(Enum(LeaveStatusEnum, values_callable=lambda x: [e.value for e in x], native_enum=False), nullable=True, default=LeaveStatusEnum.pending)

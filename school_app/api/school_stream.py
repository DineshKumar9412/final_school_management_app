...
async def create_school_stream(payload: SchoolStreamCreate, db: AsyncSession = Depends(get_db)):
    # duplicate check: same school_group_id + stream_name
...
async def list_school_streams(
    ...
    search: str | None = Query(None, description="Search by stream_name, or type 'active'/'inactive' to filter by status"),
    db: AsyncSession = Depends(get_db),
):
    key = _list_key(page, limit, search)
    cached = await cache.get(key)
    if cached:
        return Result(code=200, message="School streams fetched successfully.", extra=cached).http_response()

    offset = (page - 1) * limit
    stmt = (
        select(
            SchoolStream.school_stream_id,
            SchoolStream.school_id,
            SchoolStream.school_group_id,
...
async def get_school_stream(stream_id: int, db: AsyncSession = Depends(get_db)):
    ...
    cached = await cache.get(key)
    if cached:
        return Result(code=200, message="School stream fetched successfully.", extra=cached).http_response()
...
async def delete_school_stream(stream_id: int, db: AsyncSession = Depends(get_db)):
    ...
    class_ids = (await db.execute(
        select(SchoolStreamClass.class_id).where(SchoolStreamClass.school_stream_id == stream_id)
    )).scalars().all()
